...
func get(ctx context.Context, envInfo *cmds.Agent, proxy proxy.Proxy) (*config.Node, error) {
	...
	if err := getHostFile(clientK8eControllerCert, clientK8eControllerKey, info); err != nil {
		return nil, errors.Wrap(err, clientK8eControllerCert)
	}
	...
	if err := deps.KubeConfig(kubeconfigK8eController, apiServerURL, serverCAFile, clientK8eControllerCert, clientK8eControllerKey); err != nil {
		return nil, err
	}
	...
	nodeConfig.AgentConfig.KubeletConfigDir = filepath.Join(envInfo.DataDir, "agent", "etc", "kubelet.conf.d")
...
func validateNetworkConfig(nodeConfig *config.Node) error {
	...
	if nodeConfig.AgentConfig.ServiceCIDR == nil || nodeConfig.AgentConfig.ServiceNodePortRange.Size == 0 {
		return fmt.Errorf("incompatible down-level server detected; servers must be upgraded to at least %s", version.Version)
	}
