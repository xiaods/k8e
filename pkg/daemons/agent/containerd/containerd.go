...
func setupContainerdConfig(ctx context.Context, cfg *config.Node) error {
	...
	if err == nil {
		...
		containerdTemplate = ContainerdConfigTemplate
	} else {
		return err
	}
	parsedTemplate, err := ParseTemplateFromConfig(containerdTemplate, containerdConfig)
	if err != nil {
		return err
	}

	return fileutil.WriteFile(cfg.Containerd.Config, parsedTemplate)
}

func getPrivateRegistries(ctx context.Context, cfg *config.Node) (*Registry, error) {
