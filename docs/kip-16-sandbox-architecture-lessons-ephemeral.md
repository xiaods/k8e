...
- **M2 slice 1 — real mtimes + `ListFiles(since)` (issue #511 foundation)**: sandboxd `fileMtime` uses the `statx` syscall (was hardcoded 0), `GET /files/list?since=` filters changed files; gRPC `ListFilesRequest.since` + `k8e-sandbox-cli list --since <ts>`. This unblocks diff/incremental snapshots. Tests: Zig `fileMtime` (Linux CI), Go `TestListFiles_ForwardsSince`.

Remaining waves: M1 workspace-session reuse, M2 layerstack CAS + delta layers, M5 observability.
